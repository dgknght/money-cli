...
export function buildQualifiedName(account, accountsById) {
  ...
  while (current) {
    ...
    current = current.parent_id ? accountsById[current.parent_id] : null;
  }
  return pathSegments.join('/');
}

export function groupAndSortAccounts(accounts) {
  ...
  const accountsWithPaths = accounts.map(account => ({
    account,
  ...
  accountsWithPaths.forEach(item => {
    ...
    if (groupedAccounts[type]) {
      groupedAccounts[type].push(item);
    }
  });
...
function listAccounts(args) {
  withSpinner('Fetching accounts...', async () => {
    ...
    const grouped = groupAndSortAccounts(accounts);
