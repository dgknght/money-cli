...
export const fetchEntityId = async (entityName) => {
  if (entityName == null) {
    return getOrThrow('currentEntityId')
  }
  ...
  config.set('currentEntityId', entity.id);
  return entity.id;
};
...
function showSelectedEntity() {
  withSpinner('Fetching selected entity...', async () => {
    const entityId = config.get('currentEntityId');
    if (entityId == null) {
      return null;
    }
    const entities = await fetchEntities();
    ...
    if (entity == null) {
      ...
      console.log(`Currently selected entity: ${chalk.green(entity.name)} (ID: ${entity.id})`);
    }
  }).catch(error => {
    console.log(chalk.red('ERROR'), error);
  });
